...
class Annotation extends DraggablePlotObject {
  ...
  bool isHit(PointerDownEvent event, Matrix4 transform) {
    ...
    if (event.localPosition.dx >= globalPosition.dx - halfWidth &&
        event.localPosition.dx <= globalPosition.dx + halfWidth &&
        event.localPosition.dy >= globalPosition.dy - halfHeight &&
        event.localPosition.dy <= globalPosition.dy + halfHeight) {
      return true;
    }
    return false;
